...
tocq_words <- tocq %>%
  ...
  filter(!is.na(word)) %>% #exclude value = NA
...
tocq_chapter <- tocq %>%
  mutate(booknumber = ifelse(gutenberg_id==815, "DiA1", "DiA2")) %>%
  group_by(booknumber) %>%
  mutate(chapter = cumsum(str_detect(text, regex("^chapter ", ignore_case = TRUE)))) %>% # Cumulative Sum
  ungroup() %>%
  filter(chapter > 0) %>%
...
```{r}
tocq1 <- gutenberg_download(3300, meta_fields = "author")
```

```{r}
tocq1_words <- tocq1 %>%
 mutate(booknumber = gutenberg_id) %>%
  unnest_tokens(word, text) %>%
  filter(!is.na(word)) %>% #exclude value = NA
  count(booknumber, word, sort = TRUE) %>%
  ungroup() %>%
...
tocq1_dtm <- tocq1_words %>%
  cast_dtm(booknumber, word, n)

tm::inspect(tocq1_dtm)
```
2. Run your own topic model on these books, changing the k of topics, and evaluating accuracy.
```{r}
# topic = 10
tocq1_lda <- LDA(tocq1_dtm, k = 10, control = list(seed = 1234))

tocq1_topics <- tidy(tocq1_lda, matrix = "beta")

head(tocq1_topics, n = 10)

tocq1_top_terms <- tocq1_topics %>%
  group_by(topic) %>%
  top_n(10, beta) %>%
  ungroup() %>%
  arrange(topic, -beta)
...
```{r}
# topic = 5
tocq5_lda <- LDA(tocq1_dtm, k = 5, control = list(seed = 1234))

tocq5_topics <- tidy(tocq5_lda, matrix = "beta")

head(tocq5_topics, n = 10)

tocq5_top_terms <- tocq5_topics %>%
  group_by(topic) %>%
  top_n(10, beta) %>%
  ungroup() %>%
  arrange(topic, -beta)
...
```{r}
corp1 <- corpus(tocq1, text_field = "text")
